...
async def callback_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "menu:get_otp":
        ...
        if changed:
            ...
            await query.answer("🔄 Status refreshed. No new messages.")
        ...
        if ok:
            if order:
                ...
                # Refund only if NO messages received yet!
                if len(order.sms_messages) == 0:
                    try:
            ...
            await query.edit_message_text(
                "⚠️ *Could not cancel* (already expired, completed, or invalid).",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=main_menu_keyboard(),
            )
...
async def handle_service(query, ctx) -> None:
    ...
    db.debit_wallet(user_id, price_val, f"Purchase {service_name} ({country_code})")

    await query.edit_message_text(
        f"📱 Purchasing phone number for *{service_name.title()}*...\n"
        f"💰 Wallet Balance: `₹{db.get_user_balance(user_id):.2f}`",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def _otp_flow(
    ...
    while attempts < max_attempts:
        ...
        try:
            ...
            break # Got number successfully!
        except Exception as e:
            err_str = str(e).upper()
            is_provider_error = any(x in err_str for x in [
                "NO_NUMBERS", "NO_NUMBER", "TRY_AGAIN", "TRY AGAIN", "LIMIT", "WAIT", "AVAILABLE"
            ]) or "ACCESS" not in err_str

            if is_provider_error and attempts < max_attempts:
                if retry_msg is None:
                    retry_msg = await bot.send_message(
                        chat_id,
                        f"⏳ *No number available.* Retrying... (Attempt {attempts}/{max_attempts})",
                        parse_mode=ParseMode.MARKDOWN
                    )
                else:
                    try:
                ...
                db.credit_wallet(user_id, price_val, f"Refund: Failed purchase {service_name}")
                await bot.send_message(
                    chat_id,
                    f"❌ *Could not purchase number:* {e}\n\n"
    ...
    if not act_id or not phone:
        # Emergency backup refund
        db.credit_wallet(user_id, price_val, f"Refund: Failed purchase {service_name}")
        await bot.send_message(
            chat_id,
            f"❌ *All {max_attempts} retry attempts failed* to secure a number.\n\n"
    ...
    # Loop exited (number expired / closed)
    if sms_count > 0:
        await bot.send_message(
            chat_id,
            f"✅ *Session complete.*\n"
        ...
        # Expired with no SMS -> refund!
        db.credit_wallet(user_id, price_val, f"Refund: Expired {service_name}")
        await bot.send_message(
            chat_id,
            f"⏳ Number `+{phone}` expired with no messages received.\n\n"
            f"💰 `₹{price_val:.2f}` has been automatically refunded to your wallet.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=main_menu_keyboard(),
        )
...
def _format_order_status(order: storage.ActiveOrder) -> str:
    ...
    if order.is_cancelled:
        ...
        status_emoji = "🟡"
        status_text  = "Expired / Closed"
    elif order.cancel_requested:
        status_emoji = "🟠"
        status_text  = "Cancellation Requested"
    elif order.sms_messages:
        status_emoji = "📩"
        status_text  = "Messages received"
    ...
    return (
        ...
        f"ℹ️ *Status:* {status_emoji} {status_text}\n\n"
        f"📩 *Received Messages:*\n"
        f"{sms_block}"
    )
